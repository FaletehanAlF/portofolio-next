...
const Shuffle = ({
  ...
  // Lazily check font status so server/client initial render matches (no hydration mismatch).
  ...
  const onCompleteRef = useRef(onShuffleComplete);

  useEffect(() => {
    onCompleteRef.current = onShuffleComplete;
  }, [onShuffleComplete]);
  ...
  // NOTE: intentionally minimal base classes — sizing/weight/case come from `className`
  // so host sections (e.g. hero) keep their own typography. The upstream React Bits
  // default (`uppercase text-[4rem] leading-none`) would override the design.
