...
export default function ShapeGrid({
  ...
  useEffect(() => {
    ...
    const drawCirclePath = (cx, cy, size) => {
      ctx.beginPath();
      ctx.arc(cx, cy, size / 2, 0, Math.PI * 2);
      ctx.closePath();
    };

    const drawTrianglePath = (cx, cy, size, flip) => {
      ctx.beginPath();
      if (flip) {
    ...
    const pushTrail = (cell) => {
      if (hoverTrailAmount <= 0 || !cell) return;
    ...
    const drawGrid = () => {
      ...
      if (isHex) {
        ...
        for (let col = -2; col < cols; col++) {
          for (let row = -2; row < rows; row++) {
            ...
            if (alpha) {
              ctx.globalAlpha = alpha;
              drawTrianglePath(cx, cy, squareSize, flip);
              ctx.fillStyle = hoverFillColor;
              ctx.fill();
              ctx.globalAlpha = 1;
            }
        ...
        for (let col = -2; col < cols; col++) {
          for (let row = -2; row < rows; row++) {
            ...
            if (alpha) {
              ctx.globalAlpha = alpha;
              drawCirclePath(cx, cy, squareSize);
              ctx.fillStyle = hoverFillColor;
              ctx.fill();
              ctx.globalAlpha = 1;
            }
      ...
      // NOTE: tidak ada radial gradient per-frame di sini (sengaja).
      // Fade/vignette ditangani via overlay CSS di Hero agar 60fps mulus.
    };
    ...
    const handleMouseLeave = () => {
      if (hoveredCellRef.current) pushTrail(hoveredCellRef.current);
    ...
    const tryStart = () => {
      if (prefersReduced) {
        // Render satu frame statis saja, tanpa loop.
        updateCellOpacities();
        drawGrid();
        return;
      }
